...
/**
 * Regex pattern for layouts directive. {{!< layouts }}
 */
...
/**
 * Defines content for a named block declared in layouts.
 *
 * @example
 *
...
/**
 * Returns the layouts filepath given the template filename and layouts used.
 * Backward compatible with specifying layouts in locals like 'layouts/foo',
 * but if you have specified a layoutsDir you can specify layouts in locals with just the layouts name.
 *
...
/**
 * Find the path of the declared layouts in `str`, if any
 *
 * @param  {String} str The template string to parse
 * @param {String} filename Path to template
 * @returns {String|undefined} Returns the path to layouts.
 */
...
/**
 * Compiles a layouts file.
 *
 * The function checks whether the layouts file declares a parent layouts.
 * If it does, the parent layouts is loaded recursively and checked as well
 * for a parent layouts, and so on, until the top layouts is reached.
 * All layouts are then returned as a stack to the caller via the callback.
 *
 * @param {String}      layoutFile  The path to the layouts file to compile
 * @param {Boolean}     useCache    Cache the compiled layouts?
...
ExpressHbs.prototype.cacheLayout = function(layoutFile, useCache, cb) {
  ...
  fs.readFile(layoutFile, 'utf8', function(err, str) {
    ...
    //  File path of eventual declared parent layouts
    var parentLayoutFile = self.declaredLayoutFile(str, layoutFile);

    // This function returns the current layouts stack to the caller
    ...
    if (parentLayoutFile) {
      ...
      // No parent layouts: return current layouts with an empty stack
      _returnLayouts([]);
    }
  });
};
 ...
 *   defaultLayout: "path to default layouts",
...
ExpressHbs.prototype.express3 = function(options) {
  ...
  // Isn't there a chance block and content  are not in sync. The template and layouts are processed asynchronously.
  this.blocks = {};

  // Holds the default compiled layouts if specified in options configuration.
...
/**
 * Tries to load the default layouts.
 *
...
ExpressHbs.prototype.___express = function ___express(filename, source, options, cb) {
  ...
  /**
   * Allow a layouts to be declared as a handlebars comment to remain spec
   * compatible with handlebars.
   *
   * Valid directives
   *
   *  {{!< foo}}                      # foo.hbs in same directory as template
   *  {{!< ../layouts/default}}       # default.hbs in parent layouts directory
   *  {{!< ../layouts/default.html}}  # default.html in parent layouts directory
   */
  ...
  function renderTemplate(template, locals, cb) {
    ...
    async.done(function(values) {
      // Save for layouts. Block helpers are called within layouts, not in the
      // current template.
    ...
    getSourceTemplate(function(err, source, template) {
      ...
      parseLayout(source, filename, function(err, layoutTemplates) {
        ...
        if (typeof options.layout !== 'undefined' && !options.layout) {
          // If options.layouts is falsy, behave as if no layouts should be used - suppress defaults
          renderIt(null);
          ...
          // 3. Default layouts specified when middleware was configured.
