...
describe('layouts', function() {
  ...
  describe('layoutsDir', function() {
    ...
    it('should render layouts declared in markup', function(done) {
      request(app)
        .get('/fruits')
        .expect(/DECLARATIVE LAYOUT/, done);
    });

    it('should allow specifying layouts in locals without dir', function(done) {
      request(app)
        .get('/veggies')
        .expect(/PROGRAMMATIC LAYOUT/, done);
    });

    it('should still allow specifying layouts in locals with dir', function(done) {
      request(app)
  ...
  describe('options.layout', function() {
    ...
    it ('should process template-specified layouts without option', function(done) {
      var render = hbs.create().express3({
      });
      var locals = createLocals('express3', dirname);

      render(dirname + '/layout.hbs', locals, function(err, html) {
        assert.equal('<dld>dld</dld>', stripWs(html));
        done();
      });
    });

    it ('should allow options.layouts to be specified', function(done) {
      var render = hbs.create().express3({
      });
    ...
    it ('should not process template-specified layouts when options.layouts is falsy', function(done) {
      var render = hbs.create().express3({
      });
